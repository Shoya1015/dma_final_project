...
We try to replicate Figure.1, Table.1, Table.2 and Table.6 in the paper, which are especially critical results in the paper. Also, we try several extension approaches using the bootstrap method. In appendix, we replicate Arellano Bond Estimation for Table.2 because we failed to replicate due to several limitations.

In addition, we extend this research by derivation of a confidence interval using the bootstrap method.
...
Finally, we perform the instrumental variable (IV) method. The instrumental variables used in this analysis are as follows.
\begin{align}
Z_{ct} = \frac{1}{|I_c|} \sum_{{c^*} \in I_c} D_{ {c^*} t}. \tag{5}
\end{align}
Using this instrumental variable, we will perform the following 2SLS estimation.
\begin{align}
& y_{ct} = \beta D_{ct} + \sum_{j=1}^p \gamma y_{ct-j} + \alpha_c + \delta_t + \epsilon_{ct}, \\
& D_{ct} = \sum_{j=1}^q \pi_j Z_{ct-j} + \sum_{j=1}^p \phi_j y_{ct-j} + \theta_c + \mu_t + v_{ct} \tag{6}
\end{align}

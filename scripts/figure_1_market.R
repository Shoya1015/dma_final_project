...
# -----------------------------------------------------------
# 4. marketrefのラグ値作成と欠損値の除外
# -----------------------------------------------------------
# marketref の1期前から4期前までの値を作成し、いずれかが欠損している行は除外
data_f1 <- data_f1 %>%
  group_by(id) %>%
  arrange(year) %>%
  mutate(
    lag1 = dplyr::lag(marketref, 1),
    lag2 = dplyr::lag(marketref, 2),
    lag3 = dplyr::lag(marketref, 3),
    lag4 = dplyr::lag(marketref, 4)
  ) %>%
  ungroup() %>%
  filter(!is.na(lag1) & !is.na(lag2) & !is.na(lag3) & !is.na(lag4))

# -----------------------------------------------------------
# 5. 相対的な marketref 変化（基準：lag1）を計算する
# -----------------------------------------------------------
# ここでは、marketref の直前の値（lag1）を基準に、各期間での変化（差分）を求めます。
# ※ 「相対時点」は、民主化転換前後の年を示し、以下のように命名しています：
#     - t < 0：民主化転換前。t = -15～-2 の場合、dplyr::lag(marketref, abs(t)) - lag1 を計算
#     - t = -1：民主化直前の期間。ここは基準なので 0 とする
#     - t = 0：民主化転換直後。marketref - lag1 を計算
#     - t > 0：民主化転換後。dplyr::lead(marketref, t) - lag1 を計算
#
# 命名規則：
#   - 前期は "marketrefDiff_mX" （例：t = -15 → "marketrefDiff_m15"）
#   - 基準は t = -1 → "marketrefDiff_m1"（値は 0）
#   - 転換直後は t = 0 → "marketrefDiff_0"
#   - 後期は "marketrefDiff_pX" （例：t = 1 → "marketrefDiff_p1"）

# (a) 民主化転換前の期間：t = -15 ～ -2
for (t in -15:-2) {
  col_name <- paste0("marketrefDiff_m", abs(t))  # 例: t = -15 → "marketrefDiff_m15"
  data_f1 <- data_f1 %>%
    group_by(id) %>%
    arrange(year) %>%
    mutate(!!col_name := dplyr::lag(marketref, abs(t)) - lag1) %>%
    ungroup()
}

# (b) 基準期間：t = -1 は基準なので 0 とする
data_f1 <- data_f1 %>%
  mutate(marketrefDiff_m1 = 0)

# (c) 民主化転換直後およびその後の期間：t = 0 ～ 30
# まず、t = 0 の場合は現在の marketref と lag1 の差を計算
data_f1 <- data_f1 %>%
  group_by(id) %>%
  arrange(year) %>%
  mutate(marketrefDiff_0 = marketref - lag1) %>%
  ungroup()

# t > 0 の場合は dplyr::lead を使って未来の marketref を取得
for (t in 1:30) {
  col_name <- paste0("marketrefDiff_p", t)  # 例: t = 1 → "marketrefDiff_p1"
  data_f1 <- data_f1 %>%
    group_by(id) %>%
    arrange(year) %>%
    mutate(!!col_name := dplyr::lead(marketref, t) - lag1) %>%
    ungroup()
}
...
# -----------------------------------------------------------
# ※ 以下の関数は、指定した変数（例："marketrefDiff_m15" など）について、
...
# 各相対時点ごとに、対応する marketref 差分の変数名を決定し、ATETを計算する
for (i in seq_along(relative_times)) {
  t_val <- relative_times[i]
  if(t_val < 0) {
    # tが負の場合：例 t = -15 → "marketrefDiff_m15"
    col_name <- paste0("marketrefDiff_m", abs(t_val))
  } else {
    # tが0の場合は "marketrefDiff_0"、正の場合は "marketrefDiff_p{t}"
    col_name <- if(t_val == 0) "marketrefDiff_0" else paste0("marketrefDiff_p", t_val)
  }
...
# 8. 結果のプロット
# -----------------------------------------------------------
# marketref は 0～100 の値を取るため、単に変化量としてプロットする
figure_marketref <- ggplot(results_df, aes(x = RelativeTime, y = ATET)) +
  geom_line(color = "black") +
  scale_x_continuous(breaks = seq(-15, 30, 5)) +
  labs(x = "Years around Democratization", 
       y = "Change in marketref") +
  theme_bw()

# -----------------------------------------------------------
# 9. プロットの保存
# -----------------------------------------------------------
ggsave("output/figure_marketref.pdf", figure_marketref, width = 14, height = 8, units = "cm")

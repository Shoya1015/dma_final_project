...
# -----------------------------------------------------------
# 4. ラグ値の作成（Populationages1564oftota）
# -----------------------------------------------------------
# Populationages1564oftota の1期前から4期前までの値を作成し、いずれかが欠損している行は除外
data_fx <- data_fx %>%
  group_by(id) %>%
  arrange(year) %>%
  mutate(
...
# -----------------------------------------------------------
# 5. 相対的な Populationages1564oftota の変化（基準：lag1）を計算する
# -----------------------------------------------------------
# 各相対時点 t における変化を、以下のルールで計算します：
#   - t < 0：過去の値との差分 → dplyr::lag(Populationages1564oftota, abs(t)) - lag1
#   - t = -1：基準期なので 0 とする
#   - t = 0：現在の値との差分 → Populationages1564oftota - lag1
#   - t > 0：未来の値との差分 → dplyr::lead(Populationages1564oftota, t) - lag1
#
# ここでは、変数名を以下のように命名します：
#   - 過去： "age1564Diff_mX"（例：t = -15 → "age1564Diff_m15"）
#   - 基準： t = -1 → "age1564Diff_m1"（値は 0）
#   - 転換直後： t = 0 → "age1564Diff_0"
#   - 未来： "age1564Diff_pX"（例：t = 1 → "age1564Diff_p1"）
...
# -----------------------------------------------------------
# 以下の関数は、指定した変数（例："age1564Diff_m15" など）について、
...
# 各相対時点ごとに、対応する Populationages1564oftota 差分の変数名を決定し、ATT を計算する
for (i in seq_along(relative_times)) {
  t_val <- relative_times[i]
  if(t_val < 0) {
    # 例: t = -15 → "age1564Diff_m15"
    col_name <- paste0("age1564Diff_m", abs(t_val))
  } else {
    # t = 0 の場合は "age1564Diff_0"、t > 0 の場合は "age1564Diff_pX"
...
# 8. 結果のプロット
# -----------------------------------------------------------
figure_populationages1564oftota <- ggplot(results_df, aes(x = RelativeTime, y = ATT)) +
...
# 9. プロットの保存
# -----------------------------------------------------------
ggsave("output/figure_populationages1564oftota.pdf",
       figure_populationages1564oftota, width = 14, height = 8, units = "cm")

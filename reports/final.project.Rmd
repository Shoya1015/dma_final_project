...
# Compute lags for 'y' (lags 1 through 4) within each id and filter out rows with any missing lag values
data_f1 <- data_f1 %>%
  group_by(id) %>%
  arrange(year) %>%
  mutate(
...
# For t from -15 to -2, create columns 'gdpDiff_m[abs(t)]' as the difference between the lagged 'y' (by abs(t)) and lag1
for (t in -15:-2) {
...
# Compute the GDP difference for the current period (0) as the difference between 'y' and lag1
data_f1 <- data_f1 %>%
  group_by(id) %>%
  arrange(year) %>%
...
# For t from 1 to 30, create columns 'gdpDiff_p[t]' as the difference between the lead of 'y' (by t) and lag1
for (t in 1:30) {
...
for(i in seq_along(relative_times)) {
  ...
  # Set the column name based on whether the period is before (m), during (0), or after (p) treatment
  if(t_val < 0) {
